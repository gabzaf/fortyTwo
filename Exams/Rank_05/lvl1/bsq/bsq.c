...
void	allocate_map(t_bsq *bsq, char *av, FILE *file)
{
...
void	parse_header(t_bsq *bsq, FILE *file, char *av)
{
	if (fscanf(file, "%d%c%c%c\n", &bsq->lines, &bsq->empt,
				&bsq->obs, &bsq->full) != 4)
		close_on_error(bsq, av, file, -1, NULL);
	if (bsq->lines <= 0 || bsq->empt == bsq->obs || bsq->empt == bsq->full
			|| bsq->obs == bsq->full)
		close_on_error(bsq, av, file, -1, NULL);
}

char	*read_line(FILE *file, char *av, t_bsq *bsq, int i)
{
	char	*new = NULL;
	size_t	len = 0;
...
void	parse_bsq(t_bsq *bsq, char *av)
{
	...
	allocate_map(bsq, av, file);
	for (int i = 0; i < bsq->lines; i++)
	{
		char *line = read_line(file, av, bsq, i);
		int len = ft_strlen(line);
		if (i == 0)
			bsq->width = len;
		else if (i < bsq->lines - 1 && bsq->width != len)
			close_on_error(bsq, av, file, i, line);
		bsq->map[i] = line;
	}
	if (av)
		fclose(file);
